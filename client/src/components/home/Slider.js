...
const Slider = () => {
    ...
    return isFetching ? <div className="my-container h-[70vh] flex items-center justify-center">
    <Spinner /></div> : <Swiper
            pagination={{
                dynamicBullets: true
            }}
        ...
        >
            {data?.categories.length > 0 && data?.categories.map((cat, index) => (
                ...
                <div className={`slide-img`}>
                    <img src={`./images/slider/${index + 1}.jpg`} 
                    className="w-full h-full object-cover" />
                </div>
                <div className="absolute inset-o w-full h-full bg-black/50">
